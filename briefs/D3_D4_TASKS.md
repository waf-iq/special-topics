...
- **D3:** collapse 3 Qdrant collections → 1; wire the real seams (mechanical — contracts match); run Tasks 4/5/6 harnesses on the wired pipeline for D3 numbers.
  - **Note from Task 2 (Ahmed):** the blessed reranker is `bge` (`_BLESSED_RERANKER` in `rerank.py`) and its quality peak is at **candidate pool = 20** (see `reports/D3/d3_rerank.md`). When wiring the executor, set **`candidate_k=20`** in `GraphRAGExecutor` (`graphrag.py`, currently default 20 — keep it, don't bump to 30/50: deeper pools *lower* NDCG@5 and add ~1.7×–2.7× latency). `bge` adds ~1.1s p95; if `/ask` needs to be snappier, switch `_BLESSED_RERANKER` to `minilm` (+4.7pp at ~0.33s) — one-line change, no integrator action needed.
