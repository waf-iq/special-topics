...
- [x] `notebooks/03_d2_retrieval_stack.ipynb` — D2 **full-pipeline** notebook (marker flagged D2 had none; D1 shipped two). **Self-contained, in-memory, no Docker** — Week 5/6 lab style, importing the team's real modules. Covers ingest→store→search→graph: §2 real PDF ingestion (`parse_arxiv_pdfs`+`embed_chunks` on `data/raw_pdfs/`, then retrieve), §3-5 mongomock + in-memory Qdrant (`seed_collection_from_parquet`, 3 collections), §6-8 the real FastAPI `/search` (`create_app(qdrant_client=...)`+`TestClient`: `/healthz`, ×3 routes + no-leakage asserts, holdout metrics — deployed Recall@5=0.6489 = committed, NDCG@5 0.5589 vs 0.5611 within documented ANN drift), §9-11 Neo4j-or-networkx graph (reusing `seed_neo4j.seed_graph`) running the 5 documented queries (networkx reproduces `d2_cypher_examples.md`: 155/764/13, Jun Wang, cs.CL=113…) **plus 5 intermediate analytics** (prolific authors, co-authorship pairs, interdisciplinary authors, topic co-occurrence, most-collaborative authors). Neo4j via `.env` (`load_dotenv()`; accepts Aura's `NEO4J_URI`/`NEO4J_USERNAME`). **Executed clean via nbconvert** (`pip install qdrant-client mongomock networkx neo4j python-dotenv pymupdf`).
