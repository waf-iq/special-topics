...
`src/csai415/api.py`, `src/csai415/qdrant_dense.py`, `scripts/{ingest_arxiv_batch,seed_mongo,seed_qdrant,seed_neo4j,eval_search_metrics,capture_cypher_outputs}.py`, `cypher/01..05_*.cypher`, `docker-compose.yml`, `Makefile`, `configs/winning_runcard.yaml`, `reports/D2/{d2_dataflow,d2_int1_verification,d2_int2_verification,d2_search_metrics,d2_topk_examples,d2_cypher_examples}`. Self-contained full-pipeline notebook: `notebooks/03_d2_retrieval_stack.ipynb` (Week 5/6 lab style — in-memory, no Docker: real PDF ingestion via `ingest.parse_arxiv_pdfs`/`embed_chunks` → mongomock + in-memory Qdrant → the real FastAPI `/search` via TestClient → Neo4j-or-networkx graph; runs `/healthz`, `/search` ×3 routes, reproduces the holdout metrics, and 10 graph queries — the 5 documented + 5 intermediate analytics; set `NEO4J_URI`/`NEO4J_USERNAME`/`NEO4J_PASSWORD` in `.env` for live Cypher). AI logs per member at `ai_logs/<name>_d2.{md,txt}`.
